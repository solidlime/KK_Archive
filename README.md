...
**圧縮形式**: PNG 画像データ（IEND チャンクまで）はそのまま保持し、後続のゲームデータ部分のみを LZMA 圧縮。元の PNG 画像部分は無変更のため、エクスプローラのサムネイル表示には影響しない。
...
- キャラカードで約 **4-25%** 削減（9件の実測、平均 8.2%）
- コーデで約 **0-9%** 削減（6件の実測、平均 4.3%）
- シーンファイルで約 **5-90%** 削減（12件の実測、平均 約 34%。ファイルの内容により削減率は大きく変動）
- バイト単位で完全復元（ロスレス）
